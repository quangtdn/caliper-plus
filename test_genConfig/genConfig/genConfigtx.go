...
func GenConfigtx(domainName string, numOrgs int, numOrderer int, numKafka int) (TopLevel, error){
  ...
  topProfile["OrgsOrdererGenesis"] = &profGenesis
  topProfile["OrgsChannel"] = &profChannel
...
func GenOrderer(numOrderer int, domainName string, kafka Kafka) (Orderer, error) {
  ...
  if numOrderer == 1 {
    ...
    orderer = Orderer{
      ...
      BatchSize:    BatchSize{
        MaxMessageCount:  10,
        AbsoluteMaxBytes: 128 * 1024 * 1024, // 128 MB
        PreferredMaxBytes:  128 * 1024 * 1024, // 128 MB
      },
...
func GenKafka(numKafka int, domainName string) (Kafka, error) {
  ...
  for i := 0; i < numKafka; i++ {
    kafka_list = append(kafka_list, "kafka" + strconv.Itoa(i) + ":9092")
  }
