...
ORDERERNODE=10
NETWORKNAME="my-net"
WORKINGDIR="$HOME/kafka-network/16peer1ord/deployment"
PEERLINK=""
...
#start orderer
ssh slave-$ORDERERNODE "cd $WORKINGDIR && docker run --rm -it -d --network=$NETWORKNAME --name orderer.example.com -p 7050:7050 -e ORDERER_GENERAL_LOGLEVEL=debug -e ORDERER_GENERAL_LISTENADDRESS=0.0.0.0  -e ORDERER_GENERAL_GENESISMETHOD=file -e ORDERER_GENERAL_GENESISFILE=/etc/hyperledger/configtx/orderer.block -e ORDERER_GENERAL_LOCALMSPID=OrdererMSP -e ORDERER_GENERAL_LOCALMSPDIR=/etc/hyperledger/msp/orderer/msp -e CORE_VM_DOCKER_HOSTCONFIG_NETWORKMODE=$NETWORKNAME -w /opt/gopath/src/github.com/hyperledger/fabric -v $WORKINGDIR/../network-config:/etc/hyperledger/configtx -v $WORKINGDIR/../crypto-config/ordererOrganizations/example.com/orderers/orderer.example.com/msp:/etc/hyperledger/msp/orderer/msp hyperledger/fabric-orderer:1.1.0 orderer "
